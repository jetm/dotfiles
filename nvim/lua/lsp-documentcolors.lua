...
local function lsp_color_to_hex(color)
    ...
    return bit.tohex(bit.bor(bit.lshift(to256(color.red), 16),
...
local function make_highlight_name(rgb, mode)
    return table.concat(
               {HIGHLIGHT_NAME_PREFIX, HIGHLIGHT_MODE_NAMES[mode], rgb}, "_")
end
...
local function create_highlight(rgb_hex, options)
    ...
    if mode == "foreground" then
        vim.api.nvim_command(string.format("highlight %s guifg=#%s",
                                           highlight_name, rgb_hex))
    else
        ...
        vim.api.nvim_command(string.format("highlight %s guifg=%s guibg=#%s",
                                           highlight_name, fg_color, rgb_hex))
    end
...
local function buf_set_highlights(bufnr, colors, options)
    ...
    for _, color_info in pairs(colors) do
        ...
        local end_col = options.single_column and start_col + 1 or
                            range["end"].character

        vim.api.nvim_buf_add_highlight(bufnr, NAMESPACE, highlight_name, line,
                                       start_col, end_col)
    end
end
...
function M.update_highlight(bufnr, options)
    ...
    vim.lsp.buf_request(bufnr, "textDocument/documentColor", params,
                        function(err, _, result, _, _, _)
...
function M.buf_attach(bufnr, options)
    ...
    local trigger_update_highlight, timer =
        require"defer".debounce_trailing(M.update_highlight,
                                         options.debounce or 200, false)
